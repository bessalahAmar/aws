...
Direct Connect : AWS Direct Connect lets you establish a dedicated network connection from your network to AWS.
...
Cognito : Amazon Cognito is a simple user identity and data synchr­oni­zation service that helps you securely manage and synchr­onize app data for your users across their mobile devices.
...
Directory Service : AWS Directory Service provides managed direct­ories in the cloud.

WAF : AWS WAF (Web Applic­ation Firewall) protects web applic­ations from attack by providing web traffic filtering against common web exploits like SQL injection.

GameLift : Deploy and scale session-based multiplayer games in the cloud in minutes, with no upfront costs. 	
...
Import­/Export Snowball : AWS Import/Export Snowball accelerates moving large amounts of data into and out of AWS using secure appliances for transport.
...
Config : AWS Config gives you inventory of your AWS resources, lets you audit resource configuration history, and notifies you when resource config­ura­tions change.

Service Catalog : AWS Service Catalog allows organizations to manage approved catalogs of IT resources and make them available to employees via a person­alized portal.

CloudF­orm­ation : AWS CloudFormation lets you create and update a collection of related AWS resources in a predic­table fashion.

OpsWorks : AWS OpsWorks is a DevOps platform for managing applic­ations of any scale or complexity on the AWS cloud.

Trusted Advisor : AWS Trusted Advisor inspects your AWS enviro­nment and finds opport­unities to save money, improve system perfor­mance and reliab­ility, or help close security gaps.
...
SWF : Amazon Simple Workflow (SWF) coordinates all of the processing steps within an applic­ation.

CloudS­earch : AWS CloudSearch is a fully managed search service for websites and apps.
...
Elasti­Cache : Amazon ElastiCache improves application performance by allowing you to retrieve information from an in-memory caching system.
...
DynamoDB : Amazon DynamoDB is a scalable NoSQL data store that manages distributed replicas of your data for high availa­bility.

Redshift : Amazon Redshift is a fast, fully managed, petabyte-scale data warehouse that makes it cost-e­ffe­ctive to analyze all your data using your existing business intell­igence tools.

CodeCommit : AWS CodeCommit is a highly scalable, managed source control service that hosts private Git reposi­tories.

CodeDeploy : AWS CodeDeploy lets you fully automate code deployments.

CodePipeline : AWS CodePipeline is a continuous delivery service that enables you to model, visualize, and automate the steps required to release your software.
...
Elasticsearch Service : Amazon Elasticsearch Service is a managed service that makes it easy to deploy, operate, and scale Elasti­cse­arch, a popular open-s­ource search and analytics engine.
